...
def info_pos(text, list):
    ...
    for word, tag in okt_pos:
        if tag in ['Number']:  #영어만하고 싶으면  'Noun' 지우기
            if (word.isdecimal()) or (word.find('년') >= 0):
                list.append(word)
    print(len(list))
    return list
...
def main():
    ...
    for i in range(1, 2):
        url = f"https://www.saramin.co.kr/zf_user/search?cat_kewd=108&company=&recruitPage=1&recruitSort=relation&recruitPageCount=10&inner_com_type=&company_cd=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C9%2C10&searchword=&show_applied=&quick_apply=&except_read=&ai_head_hunting=&mainSearch=n"
        html = url_open(url)
